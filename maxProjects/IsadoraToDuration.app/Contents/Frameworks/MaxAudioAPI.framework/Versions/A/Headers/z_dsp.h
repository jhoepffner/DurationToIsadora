...
typedef struct _dspchain
{
	...
	t_linklist *c_chain64;			// new style 64 bit chain
	...
	double c_cputime_used;			// cumulative cputime (in milliseconds) used for ticking all objects in the chain 
	double c_cputime_available;		// cumulative time (in milliseconds) since this dspchain has been started
	double c_benchtime_used;		// same as c_cputime_used but limited to vectors that benchmarking was actually turned on
	double c_benchtime_available;	// same as c_cputime_available but limited to vectors that benchmarking was actually turned on 
	void **c_chain64_array;			// used to optimize dspchain_tick
} t_dspchain;
